...
func (r *runner) beforeScenario(ctx context.Context, sc *godog.Scenario) (context.Context, error) {
	...
	if capability, missing := r.missingCapability(sc); missing {
		...
		// Both are the same outcome -- the scenario was not run because the
		// capability was not declared -- and the reason is where they differ. The
		// reason is what a consumer of the report reads, so it has to carry the
		// distinction: "this provider does not declare it" describes a choice the
		// provider made, and a capability the Go SDK cannot express was never the
		// provider's to choose. Saying the first when the second is true attributes
		// a defect to a provider that has none.
		if inexpressible {
			r.noteSkipReason(sc.Id, fmt.Sprintf(
				"requires capability %s, which the Go SDK cannot express (%s), so no provider "+
					...
					"under test", capability.Tag(), reason))
			return ctx, fmt.Errorf(
