...
func scanPackage(dir string) (tests []string, runsSuite map[string]bool, err error) {
	...
	// This package's one suite calls tck.Run directly, but an adoption with two
	// resolvers factors the call into a helper, so follow same-package calls to
	// a fixed point rather than only looking inside the test function itself.
