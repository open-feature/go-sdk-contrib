...
If you are upgrading from `0.x.x`, the safest migration is to keep the old runtime behavior first and then switch to the new default intentionally.

1. Upgrade the dependency to the new provider version.
2. Start with `REMOTE` mode so the provider still evaluates flags through the relay-proxy.
3. Rename or remove old options that no longer exist.
4. Once production behavior is validated, remove the explicit `REMOTE` setting to adopt the new default `INPROCESS` mode.
5. Tune polling and data-collector settings for your production traffic profile.

If you want to keep the old evaluation pattern during the first upgrade step, use:
