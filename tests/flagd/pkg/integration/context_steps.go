...
func initializeContextSteps(ctx *godog.ScenarioContext, state *TestState) {
	...
	ctx.Step(`^a context containing a key "([^"]*)" with value "([^"]*)"$`, state.addContextStringValue)
...
// addContextStringValue adds a string value to the evaluation context (convenience method)
func (s *TestState) addContextStringValue(key, value string) error {
	return s.addContextValue(key, "String", value)
}
...
// removeContextKey removes a key from the evaluation context
func (s *TestState) removeContextKey(key string) {
	delete(s.EvalContext, key)
}
