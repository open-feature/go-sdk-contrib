# flagd Integration Test Step Definitions

This package provides reusable Gherkin step definitions for testing flagd providers across different resolver types (RPC, InProcess, File).
...
- **Utility Functions**: Shared conversion and helper functions in `utils.go`
...
├── flag_steps.go         # Flag evaluation testing
├── context_steps.go      # Evaluation context management
├── event_steps.go        # Provider event handling
└── testcontainer.go      # Container management
```
