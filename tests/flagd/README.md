...
1. **Unified Step Definitions (`pkg/integration/`)**: Single source of step definitions that work across all resolver types
...
```
tests/flagd/
├── pkg/integration/           # Unified step definitions
│   ├── step_definitions.go    # Main initialization and shared state
│   ├── config_bridge.go       # Bridge with existing config system
...
- **Provider Lifecycle**: Provider creation, initialization, and event handling
- **Flag Evaluation**: All flag evaluation scenarios (boolean, string, integer, float, object)
- **Context Management**: Evaluation context setup and manipulation
- **Event Handling**: Provider state changes and flag change events
...
2. **Comprehensive Coverage**: All flagd-testbed scenarios can be run
3. **Real Integration**: Uses actual flagd instances via testcontainers
4. **Maintainability**: Centralized test logic reduces duplication
